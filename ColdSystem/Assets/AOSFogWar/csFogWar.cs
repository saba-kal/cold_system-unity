...
namespace FischlWorks_FogWar
{
    ...
    public class csFogWar : MonoBehaviour
    {
        ...
        /// Removes a FogRevealer instance from the list with transform
        public void RemoveFogRevealer(Transform revealerTransform)
        {
            var newRevealers = new List<FogRevealer>();
            foreach (var fogRevealer in fogRevealers)
            {
                if (fogRevealer._RevealerTransform != revealerTransform)
                {
                    newRevealers.Add(fogRevealer);
                }
            }
            ReplaceFogRevealerList(newRevealers);
            UpdateFogField();
            UpdateFogPlaneTextureBuffer();
        }
